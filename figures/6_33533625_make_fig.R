...
# previous hit from rapley et al
# top independent snp
ind <- which(dat$bp == 33542478)
...
xmin <- min(c(dat$bp, topindsnp.bp, ref.bp))
xmax <- max(c(dat$bp, topindsnp.bp, ref.bp))
offset <- (xmax - xmin) / 20

# y limit is the lowest p-value
ymax <- max(c(-log10(dat$p), -log10(ref.p), topindsnp.p))
...
#  annotate("text", x = round(topsnp.bp + offset), y = topsnp.p, hjust = -0.5, label = topsnp.snp, size = 8) +
  annotate("text", x = round(ref.bp - 90000 ), y = -log10(ref.p), label = refsnp, size = 8) +
  annotate("text", x = topindsnp.bp + 85000, y = topindsnp.p, label = topindsnp.snp, size = 8) +
