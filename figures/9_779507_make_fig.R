...
xmin <- min(c(dat$bp, topindsnp.bp, snp2.bp, snp3.bp, ref.bp))
xmax <- max(c(dat$bp, topindsnp.bp, snp2.bp, snp3.bp, ref.bp))
offset <- (xmax - xmin) / 20

# y limit is the lowest p-value
ymax <- max(c(-log10(dat$p), -log10(ref.p), topindsnp.p, snp2.p, snp3.p))
