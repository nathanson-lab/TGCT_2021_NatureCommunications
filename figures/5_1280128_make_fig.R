...
xmin <- min(c(dat$bp, topindsnp.bp, snp2.bp, ref.bp))
xmax <- max(c(dat$bp, topindsnp.bp, snp2.bp, ref.bp))
offset <- (xmax - xmin) / 20

# y limit is the lowest p-value
ymax <- max(c(-log10(dat$p), -log10(ref.p), topindsnp.p, snp2.p))
...
#  annotate("text", x = round(topsnp.bp + offset), y = topsnp.p, hjust = -0.5, label = topsnp.snp, size = 8) +
  annotate("text", x = round(ref.bp  - 70000), y = -log10(ref.p), label = refsnp, size = 8) +
  annotate("text", x = topindsnp.bp + 75000, y = topindsnp.p, label = topindsnp.snp, size = 8) +
  annotate("text", x = snp2.bp + 75000, y = snp2.p, label = snp2.snp, size = 8) +
