...
# input data comes from conditionalAnalysis.sh
# ref snp is the reference snp that conditional analysis is performed on
args = commandArgs(trailingOnly = TRUE)

if( length(args) < 2)
{
  stop("need to provide 2 arguments: refsnp refsnp.p")
}
...
# ----- setup variables -----
refsnp <- as.character(args[1])
ref.p  <- as.numeric(args[2])
...
# ----- plot the data -----
# n is the number of levels in the gradient color part for plotting LD 
n <- 20
dat$ld.f <- 0
for( i in 1:n)
{
...
p1 <- ggplot(data = dat, aes(x=bp, y=-log10(p), shape=indp, colour=dat$ld.f)) + 
  ...
  geom_point(size=4) + 
  geom_point(x=ref.bp, y=-log10(ref.p), colour = "black", fill="blue", shape=25, size=4) +
  ...
  annotate("text", x = round(topindsnp.bp + offset), y = topindsnp.p, hjust = -0.5, label = topindsnp.snp, size = 8) +
...
print("writing image...")
png(paste(snpname, dt, "cond.png", sep="_"), width=1000, height=1200)
print(p1)
dev.off()
print("done!")
