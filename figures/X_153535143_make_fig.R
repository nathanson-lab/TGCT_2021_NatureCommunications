...
#  annotate("text", x = round(topsnp.bp + offset), y = topsnp.p, hjust = -0.5, label = topsnp.snp, size = 8) +
  annotate("text", x = round(ref.bp  - 190000), y = -log10(ref.p), label = refsnp, size = 8) +
  annotate("text", x = topindsnp.bp + 205000, y = topindsnp.p, label = topindsnp.snp, size = 8) +
